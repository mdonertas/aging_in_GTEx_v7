...
qc_check = function(mytis, outliers = NA,round = 0) {
  ...
  tpm = tpm[which(apply(tpm,1,function(x)mean(x==0)<=0.5)),]
  tpm = log2(tpm+1)
  ...
  } else{ saveRDS(tpm_qn,paste('./data/processed/expression/tpm_lm_l2_qn/',tisname,'.rds',sep='')) }}
}
alltis = list.files('data/processed/expression/tpm/')
sapply(rev(alltis),qc_check)
